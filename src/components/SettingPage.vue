  ...
  <div :class="store.setting.open ? 'z-50' : '-z-50'"
    ...
    <div
        class="relative w-[1200px] max-h-[90vh] h-[800px]
          p-1 max-w-[70vw] bg-gray-100 rounded-xl flex overflow-hidden
          shadow-xl"
    >
      <button class="absolute top-1 right-1 m-2 hover:bg-gray-200 p-1 rounded-full transition-colors" 
      ...
      <div
        ...
        <template v-for="(value,key) in routes">
          <tc-button @click="changePage(String(key))" 
                    class="text-gray-700 hover:bg-gray-300 bg-gray-200">
            {{ value.name }}
          </tc-button>
        </template>
      </div>
